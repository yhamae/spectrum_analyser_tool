...
class spectral:
    '''
    Ref URL: http://neuro-educator.com/ml10/
    '''
    def __init__(self):
        X = []
        z = []
...
#         print(z_km.cluster_centers_[:,0])
#         print(z_km.cluster_centers_[:,1])
        cluster_point = []
        for a, b in zip(list(z_km.cluster_centers_[:,0]), list(z_km.cluster_centers_[:,1])):
            cluster_point.append([a, b])
        # 解説6：SpectralClusteringを実施---------------------------------
        km=cluster.SpectralClustering(n_clusters, eigen_solver='arpack', affinity='rbf', gamma=10.0)
#         print(km)
        result_point = sc.inverse_transform(cluster_point)
        #こちらはwarningが出る
...
#         result = list(X)
        ...
        for i in range(len(list(X))):
            tmp = list(X[i])
            tmp.append(list(z_km.labels_)[i])
            result.append(tmp)
        return cluster_point, result, x, y, result_point
